...
AI 조직을 구성하고 Claude Code CLI를 활용해 멀티 에이전트 미션을 자동화하는 워크스페이스 기반 협업 대시보드입니다.
...
AI Hub는 **워크스페이스 기반**으로 AI 에이전트 조직을 생성하고 관리하며, 자연어 미션을 통해 여러 에이전트가 협력하여 업무를 수행하는 플랫폼입니다.

- **워크스페이스**를 생성하고 팀원을 초대하면, 같은 워크스페이스 내에서 모든 데이터를 공유합니다.
...
1. 로그인 화면에서 **워크스페이스 이름**을 입력합니다 (예: `my-team`)
2. 첫 번째 가입자는 자동으로 워크스페이스가 생성되며 관리자(org_admin)가 됩니다
3. 이후 같은 워크스페이스 이름으로 회원가입하면 해당 워크스페이스에 멤버로 참여합니다
4. 같은 워크스페이스의 멤버는 조직도, 에이전트, 미션 등 모든 데이터를 공유합니다
