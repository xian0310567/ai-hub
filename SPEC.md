...
> 폐쇄망 멀티유저 환경에서의 Claude 기반 분산 멀티 에이전트 오케스트레이션 시스템
...
| 서버 | 위치 | 역할 |
|------|------|------|
| **vm-server** | 폐쇄망 VM (중앙) | 공유 상태 관리 (Fastify + SQLite) |
| **electron-app** | 사용자 PC (각자) | UI + Claude CLI 실행 (Electron + Next.js) |

```
[사용자 PC - Electron App]              [VM - vm-server]
  Electron main process            ◀──▶  Fastify API (port 4000)
  └── Next.js (localhost:3000)           └── SQLite (공유 DB)
  └── Claude CLI spawn                   └── 조직도, Vault, 작업 큐
  └── OS 키체인 (personal vault)         └── 스케줄러, Fallback 데몬
  └── 작업 큐 폴링
```

### 1.2 SaaS 멀티테넌시 모델
- `organizations` 테이블이 최상위 테넌트 단위
- 사용자는 org에 소속되어 역할(role)을 가짐
- 같은 org에 속한 사용자는 조직도(divisions, workspaces, teams, agents)를 공유
- 개인 데이터(chat_logs, notifications, missions)는 user_id로 격리

### 1.3 실행 분리 원칙
```
실행은 분산 (각 사용자 PC) + 상태는 중앙 (VM)
```

| 구분 | 인터랙티브 작업 | 스케줄링 작업 |
|------|----------------|--------------|
| 트리거 | 사용자 채팅 | cron / webhook |
| 실행 위치 | 사용자 PC (Electron) | Fallback 데몬 (VM) |
| 인증 수단 | 본인 Claude Pro/Max | Anthropic API 키 (봇) |
| Claude 방식 | CLI spawn | Agent SDK |

### 1.4 작업 큐 Pull 방식
- 데몬이 중앙 서버 큐를 폴링하여 작업을 가져감 (Push 방식 X)
- 사용자 PC는 outbound HTTPS만 열면 됨
...
ai-hub/
  packages/
    vm-server/                # VM에 배포되는 Fastify API 서버
      src/
        db/
          schema.ts           # 전체 DB 스키마 (organizations ~ audit_logs)
          auth.ts             # 세션 검증 미들웨어
        routes/
          auth.ts             # 인증 (signup, signin, signout, me)
          orgs.ts             # 조직 관리 + 멤버십
    ...
    electron-app/             # 사용자 PC에 설치되는 Electron 앱
      electron/
...
### 2.2 vm-server DB 스키마 (주요 테이블)

```
organizations       -- 테넌트 (org_id가 모든 공유 데이터의 키)
org_members         -- user ↔ org 역할 매핑 (org_admin | team_admin | member)
...
→ vm-server /api/auth/signin (프록시)
→ vm-server가 session 쿠키 발급
→ electron-app이 vm_session 쿠키로 중계
→ 이후 모든 API 요청: vm_session → Cookie: session=xxx 로 vm-server에 전달
```
...
| personal | 사용자 PC OS 키체인 | 본인 GitHub PAT, SSH 키, Claude 인증 |

personal vault는 vm-server에 메타데이터만 존재 (값은 절대 서버에 오지 않음)
...
- [ ] vm-server Docker compose 설정 (postgres 전환 or SQLite 유지 결정)
- [ ] `.env` 설정 가이드 문서화

### 🔲 Phase 2 — 멀티유저 + Vault UI
- [ ] RBAC 미들웨어 완성 (현재 role 체크 일부만 구현)
- [ ] Vault 관리 UI (org/team Vault 화면 3개)
- [ ] 호스트 상태 대시보드 (idle/busy/offline 실시간 표시)
- [ ] Audit log UI (Settings → Audit Log)
- [ ] 팀 멤버십 UI

### 🔲 Phase 3 — Personal Vault + OS 키체인
- [ ] electron-app에 keytar 통합
- [ ] CLI 마법사 (`aihub vault add`)
- [ ] personal vault → 작업 실행 시 환경변수 주입

### 🔲 Phase 4 — Fallback 데몬 + 스케줄러
...
- vm-server DB: SQLite 유지 vs PostgreSQL 전환 (현재 SQLite)
...
3. 시크릿은 Vault 추상화로 묶이며 스코프(org/team/personal)에 따라 물리적으로 다른 곳에 저장
4. Personal Vault는 사용자 PC OS 키체인에만 존재, 서버는 메타데이터조차 모름
5. 작업 큐는 Pull 방식. 사용자 PC는 outbound HTTPS만 열면 됨
6. Audit log는 1일차부터 포함
7. 권한 모델은 처음부터 4역할로 세분화
8. Electron은 서버 실행 + 트레이 역할만. 대시보드는 Electron 창으로 렌더링
9. Fallback 데몬은 Claude Code CLI가 아닌 Anthropic API + Agent SDK
10. GitHub 푸시는 GitHub App + 봇 신원을 기본값으로
