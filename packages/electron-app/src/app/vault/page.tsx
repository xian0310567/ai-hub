...
interface Vault {
  id: string; name: string; scope: string; team_id?: string;
  description: string; expires_at?: number; created_at: number;
}
interface Secret { id: string; key_name: string; created_at: number; updated_at: number; }
...
export default function VaultPage() {
  ...
  const canWrite = userRole === 'org_admin' || userRole === 'team_admin';
  const canDeleteVault = userRole === 'org_admin';
  ...
  const addSecret = async () => {
    ...
    setSaving(true); setError('');
    const r = await fetch(`/api/vaults/${selected.id}/secrets`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const deleteSecret = async (secretId: string) => {
    if (!selected) return;
    await fetch(`/api/vaults/${selected.id}/secrets/${secretId}`, { method: 'DELETE' });
