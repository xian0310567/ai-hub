/**
 * OpenClaw 실행 인터페이스 (standalone)
 *
 * electron-app/vm-server 없이 독립적으로 OpenClaw + Claude CLI를 실행한다.
...
import { findOpenClawBinary } from './gateway-manager.js';
import { isGatewayReady } from './openclaw-client.js';
...
export async function executeWithFallback<T>(
  ...
  if (await isGatewayReady()) {
    try {
      ...
      console.warn(`[standalone-openclaw] OpenClaw 실패 (${context}), 폴백 실행:`, err);
      return fallbackFn();
    }
  }
  return fallbackFn();
...
export async function agentRun(params: AgentRunParams): Promise<AgentResult> {
  if (await isGatewayReady()) {
    try {
      ...
      console.warn(`[standalone-openclaw] Gateway 에이전트 실행 실패 (${result.error}), CLI 폴백`);
    } catch (err) {
      console.warn('[standalone-openclaw] Gateway 에이전트 실행 에러, CLI 폴백:', err);
    }
  }
